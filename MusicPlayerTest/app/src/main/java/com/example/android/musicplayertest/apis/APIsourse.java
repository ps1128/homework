...
import com.example.android.musicplayertest.activitys.ACache;
import com.example.android.musicplayertest.MYapplication;
import com.example.android.musicplayertest.activitys.SharedPreferenceUtil;
            ...
            public okhttp3.Response intercept(Chain chain) throws IOException {
                ...
                if (toRefreshToken != null && !toRefreshToken.isEmpty()) {//如果旧token存在，说明已经登录了
                    if (token != null && !token.isEmpty()) {//token不为空时说明token没有过期
                        ...
                        com.example.administrator.mdf.apis.User authAPI = new Retrofit.Builder()
                                .baseUrl(MYapplication.instanceUrl)
                                .addConverterFactory(GsonConverterFactory.create())
                                .build()
                                .create(com.example.administrator.mdf.apis.User.class);
